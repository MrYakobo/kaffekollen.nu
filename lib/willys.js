    ...
    function cb(i) {
        ...
        request({
            ...
            if (j != null) {
                ...
                fs.writeFileSync('willys.json', JSON.stringify({
                    results: totalbody
                }))
            }
        })
...
function logic() {
    ...
    db.results.forEach((o) => {
        if (!o.outOfStock && !/.*filter.*/ig.test(o.name)) {
            ...
            var obj = {
                name: o.name,
                weight: o.displayVolume,
                price: o.priceValue,
                img: {
                    full: o.image.url,
                    preview: o.thumbnail.url
                },
                brand: o.manufacturer.replace(/CLASSIC|Arvidnordquist/ig, 'Arvid Nordquist'),
            ...
            if (typeof (promotion) == 'object') {
                obj.promo = {
                    ...
                    weight: obj.displayVolume * promotion.qualifyingCount,
    ...
    function parseType(o) {
        ...
        */
        ...
        if (/.*bönor.*?/i.test(o.name)) {
            return 'bönor'
        }
        if (/.*kok.*/i.test(o.name)) {
            return 'kok'
        }
        if (/.*snabb.*|.*Nescafé.*/i.test(o.name)) {
            return 'snabb'
        }
        if (/.*prezzo.*/.test(o.name)) {
            return 'press'
        }
        return 'brygg'
    }
