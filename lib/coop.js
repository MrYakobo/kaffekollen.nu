        ...
        if (!/.*filter.*/ig.test(data.Name[0])) {
            ...
            if (typeof (data.HasPromos) !== 'undefined' && data.HasPromos[0]) {
                ...
                obj.promo = {
                    ...
                    get start(){
                        return moment(data.Promos[0].StartAt)
                    },
                    get expire(){
                        let their = moment(data.Promos[0].EndAt)
                        if(their.diff(moment(),'months') > 6){
                            //very long expire time... return instead a date 1 week forward and see what happens. (include a notice as well that the store didn't give information)
                            return {unix: moment().add(7, 'days').unix(), calculated: true}
                        }
                        return {unix: moment(their).unix(), calculated: false} }
                }
            }
